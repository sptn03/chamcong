...
- Hỗ trợ **đăng nhập qua Google hoặc Apple** (xác thực API qua nhà cung cấp).
...
## 4. Ghi chú & phạm vi giai đoạn này

- **Đã bỏ** tính năng *số giờ tăng ca* và *Quên mật khẩu*.
- **Quy tắc GPS + Wifi:** chọn GPS / Wifi / cả hai trong cấu hình ca; nếu chọn cả hai thì **phải đúng cả hai**.
- **Ranh giới nghỉ / quên chấm:** có chấm vào mà thiếu chấm ra → *quên ra/vào ca*; không chấm vào → *ngày nghỉ*.
- **Vị trí (GPS) và Wifi** quản lý riêng thành danh mục; phương thức và lựa chọn cụ thể chốt trong cấu hình ca.
- **Chấm offline** kích hoạt khi mất mạng, luôn cần chụp ảnh + toạ độ + Admin duyệt.
- Chưa có luồng xin nghỉ phép / duyệt đơn.
- Tài liệu này **không** bao gồm: thiết kế cơ sở dữ liệu, kế hoạch test, kiến trúc kỹ thuật — sẽ làm ở bước sau.